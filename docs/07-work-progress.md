...
### 17.13 正式案例运行与影响分析质量门（2026-08-23）

- [x] 应用迁移 `20260823_0031`，为事件增加 `time_resolution`，后端已重启并以 `bridge` 作为行情数据边界。
- [x] 正式案例“美联储宣布降息50个基点”完成知识截止时间重跑：工作流
  `wfr_01a02f11-d8d0-7358-b0ba-53639fdcbaa6` 成功，事实核验、公司分析、反方审查、综合与全部 Guardrail 规则通过。
- [x] 修复 PostgreSQL 审核任务生产仓储缺少 `save_review_task` 外层事务包装的问题，避免正式工作流在最终化阶段中断。
- [x] 为影响分析增加 legacy V2 输出归一化：兼容 `source/target/relation`、旧枚举和证据引用形态，同时保持 `extra=forbid` 与证据覆盖率质量门。
- [-] 正式案例已生成包含 5 个节点、4 条边的因果图（影响分析版本 4），但模型证据覆盖率仅 `0.30`，因此保留 `draft/degraded`，不得自动批准或回填目标影响；待补齐可验证证据后由 reviewer 复核。

### 17.14 Agent 自动审核策略（2026-08-23）

- [x] 新增迁移 `20260823_0032`，持久化默认 `agent` 审核策略和自动审核尝试审计记录。
- [x] 新增管理员审核策略 API：`GET/PATCH /api/v1/admin/review-policy`；环境变量支持 `FINSIGHT_REVIEW_MODE` 默认值和 `FINSIGHT_AUTO_REVIEW_DISABLED` 紧急关闭。
- [x] 统一 `AutoReviewService` 读取运行时策略，保留置信度门槛、质量门和人工升级路径；自动决定使用 `agent:default_reviewer` 并写入不可变尝试记录。
- [x] 新增 `auto-review` Worker，回扫 pending/open 队列；Compose 增加 `auto-review-worker` 服务。当前本地 Worker 已启动，策略为 `agent`。
- [x] 管理后台“模型配置”增加审核自动化切换和 Default Reviewer/Impact Analyst/Planner 绑定项。
- [-] 影响分析 `needs_review` 的自动批准和发布签发仍需补充专用 ReviewTask 转换器；degraded 分析继续强制人工，不能绕过证据质量门。

### 17.15 审核队列状态工作台（2026-08-23）

- [x] 新增统一审核队列总览、状态计算、Agent 尝试详情和审核时间线 API。
- [x] 队列状态区分待 Agent、Agent 已决定、等待人工、已完成和 SLA 超时，并返回最近尝试、置信度、原因和任务年龄。
- [x] 审核中心增加状态汇总卡片、对象/状态筛选、Agent 结果展示、10 秒自动刷新和审核轨迹时间线。
- [x] 修复自动审核编排重复写入尝试记录的问题；全量后端测试与前端生产构建通过。
- [ ] 后续增加持久化 Worker 心跳和 merge review 统一进入前端队列的完整展示。

### 17.16 审核队列对象可读化与专业工作台（2026-08-23）

- [x] 统一审核队列返回业务对象投影：标题、类型、副标题、摘要、关联事件、跳转地址和可复制技术 ID；对象缺失时提供降级展示。
- [x] 将报告、工作流、事实冲突和事件合并审核纳入统一队列查询，保留各自原有详情与决策接口。
- [x] 增加风险等级、优先级分数、优先级解释及服务端状态/对象/风险/排序筛选；默认按优先级降序返回。
- [x] 审核中心改为业务对象优先展示，增加关联事件、风险、Agent 结果、SLA、复制 ID、手动刷新和响应式样式。
- [x] 修复前端状态/风险筛选实际未应用的问题；前端构建、单元测试、后端 lint 与运行接口验证通过。
- [ ] 后续将对象摘要解析下沉为批量查询/read model，避免大规模队列的跨对象 N+1 读取；增加统一队列详情时间线对 merge review 的展示。

### 17.17 审核队列汇总信息视觉重构（2026-08-23）

- [x] 将原有无层级的统计面板改为统一“队列概览”容器，突出总任务、人工待办、SLA 超时和已完成数量。
- [x] 增加状态分布、占比、状态色点、比例条和可点击状态筛选，汇总信息与审核列表联动。
- [x] 增加最早待处理、人工待办、SLA 超时率和刷新状态展示，支持手动刷新与自动刷新反馈。
- [x] 增加桌面端、平板端和移动端响应式布局，避免汇总区域溢出或卡片高度不一致。
- [x] 为待 Agent、处理中、Agent 已决定、等待人工、已完成和 SLA 超时分别设置颜色、边框、背景、状态点与进度条样式；列表徽章同步采用状态语义色。
- [x] 修复“全部状态”列表将状态编码提前转换为中文导致样式选择器失效的问题；列表行、状态徽章与汇总项统一使用稳定状态编码渲染。

### 17.18 事件证据影响分析空状态处理（2026-08-23）

- [x] 修复事件尚未生成影响分析时前端直接显示“加载影响分析失败”的问题。
- [x] 将后端 `IMPACT_ANALYSIS_NOT_FOUND` 视为正常空状态，继续展示手动生成入口；真正的网络、权限和服务端错误仍保留错误提示。

### 17.19 通用事件能力包 Runtime 第一阶段（2026-08-24）

- [x] 新增版本化 `CapabilityPackManifest`、能力包注册表、生命周期状态与事件类型/子类型解析。
- [x] 新增 `capital-markets.equity-financing@1.0.0` 能力包，声明事实字段、研究问题、传导机制、工具白名单和质量门。
- [x] 分类器支持股权融资事件结构化抽取；阿里配售案例可识别发行主体、融资方式、公告阶段、募集用途、金额/币种和超额认购状态。
- [x] 事件持久化能力包 ID/版本，API 返回能力包元数据；实体解析接入可替换的版本化参考数据提供器并保留代码解析回退。
- [x] Planner 将能力包版本与编译后的研究任务元数据写入 ResearchPlan，动态工作流保留旧模板执行兼容性。
- [x] 增加能力包生命周期与股权融资计划编译测试。
- [ ] 下一阶段：将能力包任务映射为可执行 Agent/Tool contract，接入官方公告与行情适配器，支持 shadow/canary 对比、历史重处理和多版本结果并存。

### 17.20 OOD 事件学习基础闭环（2026-08-25）

- [x] 新增 OOD 检测契约与确定性评分：区分已知类型、未知但具备金融相关性的事件和无关内容。
- [x] 新增 `OODObservation` 领域对象、SQLAlchemy 映射、InMemory/SQL 仓储和迁移 `20260825_0034`。
- [x] OOD 事件接入主 Pipeline：使用通用经济事件能力包进行初步处理，并保存分类器、Router、能力包和 `as_of` 版本信息。
- [x] 新增 OOD 观察与候选簇查询 API：`/api/v1/ood/observations`、`/api/v1/ood/clusters`。
- [x] 增加 `ood-observation` Worker 入口，形成可扩展的观察队列运行边界。
- [x] 新增 OOD 检测、Observation 持久化和非财经事件降级测试。
- [ ] 下一阶段：实现 Embedding/结构化特征快照、增量聚类、候选类型提案和 OOD 学习工作台。

### 17.21 OOD 学习扩展与能力包治理基础（2026-08-25）

- [x] 新增 OOD 特征快照、候选簇、事件类型提案、能力包评测和历史重处理 Job 领域模型、SQL 映射与仓储接口。
- [x] 增加确定性候选簇服务：按候选 Router 类型聚合 Observation，生成稳定簇 ID、凝聚度、稳定度和来源计数。
- [x] 增加 `TypeDiscovery` 基础服务：从候选簇生成带正/反边界、字段、机制和置信度的事件类型提案。
- [x] 增加 Capability Pack Builder：生成 candidate 状态的声明式能力包，并通过统一注册表参与后续生命周期。
- [x] 增加能力包评测、能力包列表/状态转换、事件类型提案和重处理 Job API。
- [x] 增加 OOD 学习端到端测试；全量后端测试通过。
- [ ] 后续增强：接入真实 Embedding/向量库、LLM TypeDiscovery、可执行 Workflow Compiler、Shadow/Canary 流量运行和真实重处理 Worker。

### 17.22 企业级目标影响与维度聚合（2026-08-26）

- [x] 扩展 `ImpactContribution`：目标角色、关系置信度、推断类型、证据、成立/失效条件和正式/探索范围。
- [x] 新增 `ImpactDimensionContribution`，将收入、成本、利润、估值、情绪等维度作为独立贡献保存。
- [x] 影响投影升级为 `impact-aggregation-v2`，关系置信度参与有效贡献计算，并在 Dashboard 返回角色、证据和维度拆解。
- [x] 目标影响前端增加维度卡片、企业角色和关系置信度展示；保留原有瀑布图、时间线和图视图兼容性。
- [x] 新增迁移 `20260826_0035`，保留旧贡献默认字段并支持维度表幂等创建。
- [x] 影响聚合、分析、API 和前端生产构建验证通过。
- [ ] 后续增强：企业关系主数据/审核、机制中间节点图、目标层级传播、探索情景切换和真实 Shadow/Canary Worker。

### 17.23 正式/探索影响视图隔离（2026-08-26）

- [x] 目标影响 Dashboard 新增 `publication_scope=official|exploration` 查询参数。
- [x] 正式视图仅使用已批准且标记为 `official` 的贡献；探索视图允许草稿、待审核和已批准贡献，并保留来源范围标识。
- [x] 探索视图复用同一套时间衰减、关系置信度和维度聚合算法，避免生成另一套不可解释的计算逻辑。
- [x] 增加 API/聚合回归验证，默认行为保持正式视图兼容。
- [ ] 后续增强：目标层级主数据传播、探索结果独立快照存储与 Shadow/Canary Worker。

### 17.24 目标层级主数据基础（2026-08-26）

- [x] 目标定义增加 `parent_target_id` 与 `hierarchy_level`，支持企业、子行业、行业、市场等层级关系。
- [x] 增加迁移 `20260826_0036_target_hierarchy` 及父目标索引。
- [x] 目标 API 返回层级元数据，旧数据默认根层级并保持兼容。
- [ ] 下一步：增加关系有效期/审核状态、层级传播权重和跨层级聚合快照。

### 17.25 层级目标影响传播（2026-08-26）

- [x] 聚合查询目标时自动发现其下属目标，并沿父子链纳入企业/子行业贡献。
- [x] 每跨一层使用确定性 0.85 衰减，同时叠加事件依赖权重和目标关系置信度。
- [x] Dashboard 与快照重算使用同一传播规则，保留原始贡献目标 ID，支持回溯。
- [ ] 后续将传播系数配置化，并增加关系有效期、审核状态和独立层级快照。

### 17.26 层级传播生产化约束（2026-08-26）

- [x] 层级传播衰减支持环境配置 `FINSIGHT_TARGET_PROPAGATION_DECAY`，默认 `0.85`。
- [x] 按 `valid_from/valid_to` 对目标层级关系做 as-of 过滤，支持历史回放和防止未来数据泄漏。
- [x] Dashboard 与快照重算继续复用同一规则。
- [ ] 后续将配置提升为版本化策略实体，并加入关系审核状态与审计记录。

### 17.27 层级传播策略与审核元数据（2026-08-26）

- [x] 目标层级增加审核状态、来源、审核人、审核时间和单关系传播权重。
- [x] 传播仅使用 `approved` 层级关系；每条子目标可覆盖默认衰减系数。
- [x] 迁移 `20260826_0036` 完成字段与索引升级，旧数据使用安全默认值。
- [x] 层级传播、API 和影响聚合回归测试通过。

### 17.28 研究驾驶舱总览（2026-08-26）

- [x] 新增 `/api/v1/overview/research`，按时间窗口聚合近期事件、影响目标、方向、期限、置信度和概括性解释。
- [x] 正式总览仅纳入已批准分析，待处理分析计入数据质量与风险提示。
- [x] 根页面第一屏升级为研究驾驶舱，展示综合方向、正负影响、事件卡片和受影响目标入口。
- [x] 保留原运营指标区，事件接口失败时不阻断运营监控。
- [x] 完成 API 测试、Ruff 检查和前端生产构建验证。

### 17.29 事件知识图谱与动态市场 Universe（2026-08-27）

- [x] 新增 `/api/v1/market/universe`，从受治理的标的目录动态返回当前可用市场、标的和 Provider capability。
- [x] 新增 `/api/v1/events/graph`，将近期事件、主题、已审核事件关系及影响目标投影为可追溯知识图谱。
- [x] 图谱区分事件主题、因果/更新/放大/对冲关系与事件到目标的影响边；正式模式只使用已批准关系。
- [x] 首页增加交互式事件关系图谱，复用 ELK 自动布局、置信度筛选、核心路径和节点详情能力。
- [ ] 后续扩展：市场区域/资产类别主数据、全球行情聚合服务、跨供应商并发快照和事件—行情双向筛选。

### 17.30 运营指标数字展示统一（2026-08-27）

- [x] 新增可复用 `OperationalMetricCard`，统一数字层级、状态色、进度条、图标和可访问链接语义。
- [x] 首页运营总览将待审、来源异常、成功率、延迟、引用完整率和 Outbox 分为关注、告警、健康和中性状态。
- [x] 审核中心队列 KPI 复用同一指标卡，强化人工介入、SLA 超时和完成状态的视觉区分。
- [x] 完成前端 TypeScript 与生产构建验证，并支持窄屏网格降级。

### 17.31 紧凑运营指标条（2026-08-27）

- [x] 首页六项运营指标改为六列紧凑横向指标条，显著降低首屏空间占用。
- [x] 审核中心四项 KPI 压缩为紧凑网格，保留告警、进度和跳转语义。
- [x] 指标说明单行截断、状态线改为顶部细线，并适配中屏与移动端布局。
