...
EVENT_SCHEMAS: dict[str, EventSchema] = {
    ...
    "equity_financing": EventSchema(
        event_type="equity_financing",
        keywords=("配售新股", "新股配售", "配股", "增发", "定向发行", "募资"),
        importance=0.85,
        fields=(
            FieldSpec("issuer", "string", required=True, description="发行主体"),
            FieldSpec("financing_method", "string", required=True, description="融资方式"),
            FieldSpec("announcement_stage", "string", required=True, description="公告阶段"),
            FieldSpec("use_of_proceeds", "string", required=True, description="募集资金用途"),
            FieldSpec("gross_proceeds", "decimal", description="募集金额"),
            FieldSpec("currency", "string", description="币种"),
            FieldSpec("new_shares", "decimal", description="新增股份数量"),
            FieldSpec("placement_price", "decimal", description="配售价格"),
            FieldSpec("oversubscription_ratio", "decimal", description="超额认购比例"),
            FieldSpec("subscription_status", "string", description="认购状态"),
        ),
        claim_predicate="announces_equity_financing",
        claim_object_type="string",
        claim_templates=(
            ClaimTemplate("gross_proceeds", "raises_capital", "decimal"),
            ClaimTemplate("use_of_proceeds", "uses_proceeds_for", "string"),
        ),
        conflict_rules=("value", "subject", "unit", "scope"),
    ),
